...
def test_vote(test_posts, session, test_user, test_user2):
    ...
    another_vote = models.Vote(post_id=test_posts[2].id, user_id=test_user2["id"])
    session.add(new_vote)
    session.add(another_vote)
    session.commit()
...
def test_delete_other_user_vote(authorized_client, test_vote, test_posts):
